...
from utilities.losses import DC, CE, DC_CE, DC_CE_Focal, PartialLoss, SizeLoss, VarianceLoss, TightnessPrior
...
def train(paths_dict, model, transformation, criterion, device, save_path, fold_dir, logger, opt):
    ...
    dataloaders["training"] = infinite_iterable(
        DataLoader(subjects_train, batch_size=opt.batch_size, num_workers=1, shuffle=True)
    )
    dataloaders["validation"] = infinite_iterable(
        DataLoader(subjects_val, batch_size=1, num_workers=1)
    )
    ...
    #             weight_decay=weight_decay, momentum=0.99, nesterov=False)
    ...
    # loss_gatedcrf_kernels_desc = [{"weight": 1, "xy": 6, "rgb": 0.1}]  # 参数包括权重（"weight"）、空间距离（"xy"）和颜色距离（"rgb"）
    # loss_gatedcrf_radius = opt.kernel_radius  # 卷积核半径
    # down_size = opt.down_size
    #
    # # Variance Loss initialisation  方差损失
    # variance_loss = VarianceLoss()

    # DTP+Size
    SizeLoss_loss = SizeLoss()
    TightnessPrior_loss = TightnessPrior()
    ...
    while continue_training:
        ...
        for phase in PHASES:
            ...
            running_loss_SizeLoss = 0.0
            running_loss_TightnessPrior = 0.0
            running_dice = 0.0
            epoch_samples = 0
            ...
            for _ in tqdm(range(nb_batches[phase])):
                ...
                with torch.set_grad_enabled(phase == "training"):
                    ...
                    # Segmentation loss  使用测地线伪标签作为参照   标准 ; 准则 ; 原则
                    loss_seg = criterion(outputs, geodesic_labels, phase)
                    # gated crf loss  使用原图和预测计算的loss   条件随机场loss
                    if phase == "training":
                        ...
                        # #####
                        # loss_crf_HWD = \
                        # gated_crf_loss(torch.softmax(outputs_down_HWD, dim=1), loss_gatedcrf_kernels_desc, \
                        #                loss_gatedcrf_radius, inputs_down_HWD, down_size[0], down_size[1], down_size[2])[
                        #     "loss"]
                        # outputs_down_DHW = torch.permute(outputs_down_HWD, (0, 1, 4, 2, 3))
                        # inputs_down_DHW = torch.permute(inputs_down_HWD, (0, 1, 4, 2, 3))
                        # ######
                        # loss_crf_DHW = \
                        # gated_crf_loss(torch.softmax(outputs_down_DHW, dim=1), loss_gatedcrf_kernels_desc, \
                        #                loss_gatedcrf_radius, inputs_down_DHW, down_size[2], down_size[0], down_size[1])[
                        #     "loss"]
                        # outputs_down_WDH = torch.permute(outputs_down_HWD, (0, 1, 3, 4, 2))
                        # inputs_down_WDH = torch.permute(inputs_down_HWD, (0, 1, 3, 4, 2))
                        # #####
                        # loss_crf_WDH = \
                        # gated_crf_loss(torch.softmax(outputs_down_WDH, dim=1), loss_gatedcrf_kernels_desc, \
                        #                loss_gatedcrf_radius, inputs_down_WDH, down_size[1], down_size[2], down_size[0])[
                        #     "loss"]
                        # loss_crf = (loss_crf_HWD + loss_crf_DHW + loss_crf_WDH) / 3.0  ######三个轴位的损失
                        # #####
                        # loss_variance = variance_loss(outputs, inputs)  # 方差损失

########################################################################
                        loss_SizeLoss = SizeLoss_loss(outputs, inputs)
                        loss_TightnessPrior = TightnessPrior_loss(outputs, inputs)

 #####################################################################
                        # PAseg-loss
                        # loss = loss_seg + sigmoid_rampup(epoch, opt.rampup_epochs) * (
                        #             opt.weight_gatedcrf * loss_crf + opt.weight_variance * loss_variance)

#####################################################################
                        # DTP
                        loss = loss_seg + loss_SizeLoss  + loss_TightnessPrior
 ##################################################################
                    else:
                        # loss_crf = torch.Tensor([0]).to(device)
                        # loss_variance = torch.Tensor([0]).to(device)
                        # loss = loss_seg + loss_crf + loss_variance  # PAseg
                        # ###########################################################
                        loss_SizeLoss = torch.Tensor([0]).to(device)
                        loss_TightnessPrior = torch.Tensor([0]).to(device)
                        loss = loss_seg + loss_SizeLoss + loss_TightnessPrior   # DTP

                    if phase == "training":
                        loss.backward()
                        optimizer.step()
                        # 首先，使用criterion计算输出outputs和测地距离标签geodesic_labels之间的交叉熵损失（loss_seg）。
                        #
                        # 接下来，判断当前阶段（phase）是否为训练阶段。如果是训练阶段，则进行以下操作：
                        #
                        # 使用F.interpolate将outputs和inputs双线性插值到指定的尺寸down_size，得到outputs_down_HWD和inputs_down_HWD。
                        # 调用gated_crf_loss函数计算outputs_down_HWD经过Gated
                        # CRF处理后的损失（loss_crf_HWD）。
                        # 对outputs_down_HWD、inputs_down_HWD、outputs_down_DHW、inputs_down_DHW、outputs_down_WDH、inputs_down_WDH进行维度转换。
                        # 分别调用gated_crf_loss函数计算outputs_down_DHW和outputs_down_WDH经过Gated
                        # CRF处理后的损失（loss_crf_DHW和loss_crf_WDH）。
                        # 计算三个维度上的Gated
                        # CRF损失的均值（loss_crf）。
                        # 调用variance_loss函数计算输出outputs的方差损失（loss_variance）。
                        # 计算总损失（loss），其中包括交叉熵损失、Gated
                        # CRF损失和方差损失，并根据当前的训练轮数和opt.rampup_epochs参数进行加权。
                        #
                        # 最后，如果当前阶段是训练阶段，则进行反向传播（loss.backward()）和优化器更新（optimizer.step()）
                ...
                running_loss_SizeLoss += loss_SizeLoss.item()
                running_loss_TightnessPrior += loss_TightnessPrior.item()
                if phase == "validation":
                    running_dice += dice
            ...
            epoch_loss_SizeLoss = running_loss_SizeLoss / epoch_samples
            epoch_loss_TightnessPrior = running_loss_TightnessPrior / epoch_samples
            ...
            writer.add_scalar("{0:}/loss_SizeLoss".format(phase), epoch_loss_SizeLoss, epoch)
            writer.add_scalar("{0:}/loss_TightnessPrior".format(phase), epoch_loss_TightnessPrior, epoch)
            if phase == "validation":
            ...
            logger.info("{}  Loss SizeLoss: {:.4f}".format(
                phase, epoch_loss_SizeLoss))
            logger.info("{}  Loss TightnessPrior: {:.4f}".format(
                phase, epoch_loss_TightnessPrior))
...
def main():
    ...
    dice_ce = DC_CE(NB_CLASSES)  # 计算准确度
    criterion = PartialLoss(dice_ce)  # 部分交叉熵
