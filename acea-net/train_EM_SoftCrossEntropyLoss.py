...
from utilities.losses import (DC, CE, DC_CE, DC_CE_Focal, PartialLoss, SizeLoss, VarianceLoss, TightnessPrior,
                              MumfordShah_Loss3D22D, SoftCrossEntropyLoss)
...
def train(paths_dict, model, transformation, criterion, device, save_path, fold_dir, logger, opt):
    ...
    while continue_training:
        ...
        for phase in PHASES:
            ...
            running_loss_SoftCrossEntropyLoss = 0.0
            ...
            for _ in tqdm(range(nb_batches[phase])):
                ...
                with torch.set_grad_enabled(phase == "training"):
                    ...
                    # print('55', inputs)
                    loss_SoftCrossEntropyLoss = sce_loss(outputs, geodesic_labels)
                    # print(geodesic_labels, outputs)
                    # gated crf loss  使用原图和预测计算的loss   条件随机场loss
                    if phase == "training":

                        loss = loss_seg + 0.5*loss_SoftCrossEntropyLoss
 ##################################################################
                    else:
                        ...
                        # ###########################################################
                        loss_SoftCrossEntropyLoss = torch.Tensor([0]).to(device)
                        loss = loss_seg + loss_SoftCrossEntropyLoss
                ...
                running_loss_SoftCrossEntropyLoss += loss_SoftCrossEntropyLoss.item()
            ...
            epoch_loss_SoftCrossEntropyLoss = running_loss_SoftCrossEntropyLoss / epoch_samples
            ...
            writer.add_scalar("{0:}/loss_SoftCrossEntropyLoss".format(phase), epoch_loss_SoftCrossEntropyLoss, epoch)
            ...
            logger.info("{}  Loss SoftCrossEntropyLoss: {:.4f}".format(
                phase, epoch_loss_SoftCrossEntropyLoss))
